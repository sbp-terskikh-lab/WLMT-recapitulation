...
#for(i in 2:nrow(names6m)) # move 1 idx down since pandas insers name at pos 1,1
# added header = true since it seems that is needed
print(system.time({  data0 <- fread(names6m[2,1], header = TRUE, stringsAsFactors = FALSE) }))
# print(system.time({  data0 <- fread(paste0("zcat ", names6m[1,1]),stringsAsFactors = FALSE) }))
samplename <- substr(names6m[2,1],8,str_length(names6m[2,1])-10)
...
#for(i in 2:nrow(names6m)) # move 1 idx down since pandas insers name at pos 1,1
for(i in 3:nrow(names6m))
  {
  data1 <- fread(names6m[i,1], header = TRUE, stringsAsFactors = FALSE)
...
write.csv(d0_covered_5_90,list_of_sites_with_good_coverage)